...
def find_audio_for_stem(audio_dir: Path, stem: str, audio_exts: List[str]) -> Optional[Path]:
    ...
    hits = sorted(audio_dir.glob(f"{stem}.*"))
    for p in hits:
...
def collect_pairs(datasets_root: Path, audio_exts: List[str], max_files_per_db: Optional[int]) -> List[Pair]:
    ...
    #   A) datasets_root/DB1/{bvh/,audio/}, datasets_root/DB2/{bvh/,audio/} ...
    #   B) datasets_root/{bvh/,audio/} (single DB)
    root_bvh = find_child_dir(datasets_root, ["bvh", "BVH", "bvhs", "BVHS"])
    root_audio = find_child_dir(datasets_root, ["audio", "AUDIO", "wav", "WAV", "music", "MUSIC"])

    if root_bvh and root_audio:
        db_folders = [datasets_root]
    else:
...
def main() -> None:
    ...
    lma_export_py = repo_root / "scripts" / "lmaTempoExport.py"
    if not lma_export_py.exists():
        # fallback: allow running from the same folder as this script
        alt = Path(__file__).resolve().parent / "lmaTempoExport.py"
        if alt.exists():
            lma_export_py = alt
        else:
            raise SystemExit(
